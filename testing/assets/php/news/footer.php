<footer>
      <div class="footer1">
      <a href="#">&copy UEP APEP "La Candelaria" - Algunos derechos reservados.</a>
      </div>
      <div class="footer2">
      <a href="#">Desarrollado por Recker</a>
      </div>
    </footer>
    <!-- Popad -->
    <div id="popad">
      <div id="popad-caja">
        <div id="popad-title">
          <span id="popad-titulo"></span>
          <span id="popad-cerrar">X</span>
        </div>
        <div id="popad-caja2">
          <span id="popad-info"></span>
        </div>
      </div>
    </div>
    <!-- Cookies -->
    <div id="cookies">
		  <span>Este sitio web utiliza cookies propias para que usted tenga una mejor experiencia de usuario. Si continúa navegando está dando su consentimiento para la aceptación de las mencionadas cookies y la aceptación de nuestra <span class="cookie_link">política de cookies</span>.</span>
		  <a href="javascript:void(0)" class="ok" id="confirm_cookie">Aceptar politicas de cookies</a>
  	</div>
    <!-- Alerts -->
    <div id="areaAlert">
    </div>
    <!-- Lector de imagenes -->
    <div id="showImg">
      <div id='showImgOptions'>
        <span id='showImgDownload'>Descargar</span>
        <span id='showImgOptionsSeparator'></span>
        <span id='showImgClose'>Cerrar</span>
      </div>
      <div id='show'>
        <img src="user/avatars/default.jpg" alt="imagen1">
      </div>
      <div id='showImgControl'>
        <span id='showLeft'><</span>
        <span id='showImgControlSeparator'></span>
        <span id='showRight'>></span>
      </div>
    </div>
    <script type="text/javascript" src="assets/js/news.bundle.js"></script>
	</body>
</html>