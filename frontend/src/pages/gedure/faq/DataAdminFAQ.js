...
export default function DataAdminFAQText({ sortTitle }) {
  if (sortTitle === 'Carga de estudiantes') {
    ...
    return (
      <>
        <strong>Gedure busca automáticamente</strong> al estudiante después de <strong>encontrar una cédula</strong> en el archivo <strong>PDF</strong>, esto le permite <strong>usar cualquier formato en la boleta</strong> ya que el sistema se encargará de adaptarse al formato dado. Debe tener en cuenta que las <strong>boletas</strong> tienen que tener el <strong>formato PDF</strong>, una vez cumpla esto requisitos simplemente <strong>comprima</strong> todas las boletas en el <strong>orden que quiera</strong> en un <strong>archivo ZIP</strong> y súbalo al servidor, el sistema se encargará de distribuirlas a los estudiantes existentes. También es importante aclarar que <strong>no debe preocuparse</strong> por subir <strong>boletas en cada archivo</strong> o <strong>todas en uno mismo</strong>, el sistema automáticamente separa cada hoja del PDF para procesarla.
        <br />
        <br />
