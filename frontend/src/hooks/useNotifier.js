...
export default function useNotifier({ 
  ...
  const { notiText, notiStatus } = useSelector(
    state => state.notistack,
  );
  const dispatch = useDispatch();

  useEffect(() => {
    if (notiStatus === 200) {
      messageTo200 && enqueueSnackbar(notiText, {
        variant: 'success',
      });
      ...
      messageTo422 && enqueueSnackbar('Error al verificar los datos', {
        variant: 'error',
      });
  ...
  }, [notiText, notiStatus, dispatch, enqueueSnackbar, messageTo200, messageTo422]);
}
